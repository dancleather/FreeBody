...
Calculates the parameters of the optimization, based upon optimization method 1 (as described in the FreeBody user manual)

*/
void Muscle::mus_opt_values_1(Segment **segment_data[]) {
	...
	for (int i=0; i<points-1; i++) {														// This routine iterates for each muscle point
		...
		if ((segment_dist_point == segment_most_dist) && (segment_prox_point != segment_most_dist)) {
			...
			if (num_segments == 2) {
				...
				this->opt[2][3]=-1;
				this->opt[4][3]=segment_most_prox;
				this->opt[5][3]=-1;
			...
			// intermediate segment
				...
				if (segment_most_dist != 4) { // Don't do this routine for rec_fem
					...
					for (int j=0; j<3; j++) {
						this->opt[5][j]=opt_vec[j];
					}
					this->opt[2][3]=-1;
					this->opt[5][3]=(segment_data[frame][segment_most_prox]->dist);					
				}
		...
		// intermediate segment
			...
			this->opt[1][3]=segment_most_prox;
			this->opt[2][3]=(segment_data[frame][segment_most_prox]->dist);
			this->opt[4][3]=segment_most_prox;
			this->opt[5][3]=(segment_data[frame][segment_most_prox]->dist);
